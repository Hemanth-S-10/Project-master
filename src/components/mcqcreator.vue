  ...
  <div id="app">
    <el-card style="background:violet;">
      <table>
        ...
        <td><h3><el-input type="text" v-model="data.category"/></h3></td>
        </tr>
        <tr>
        <td><h3>Question:</h3></td>
        <td><h3><el-input  type="textarea" :autosize="{ minRows: 2}"  v-model="data.question"/></h3></td>
        </tr>
        <tr>
        <td><h3>Options</h3></td>
        </tr>
        ...
        <tr v-for="i in data.opt" :key="i">
          <b>{{i}}.</b><input type="text" v-on:click="addinput()" v-model="data.incorrect_answers[i-1]"/><input type="radio" v-on:click="answer(i)">
        </tr>
      </table><br><br>
...
export default {
  ...
  methods: {
      add() {
          ...
          this.arr.push({category: this.data.category, type: this.data.type, difficulty: this.data.difficulty, question: this.data.question,
          correct_answer:this.data.correct_answer,incorrect_answers:this.data.incorrect_answers});
